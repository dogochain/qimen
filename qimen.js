...
function calculateJuShu(date, method = '时家') {
    ...
    // ==================== 詳細除錯打印 ====================
    console.log('【局數計算除錯】開始');
    console.log('   日期:', date.toLocaleString());
    console.log('   農曆:', lunar.toString());

    // 1. 取得農曆四柱
    let yearZhi, monthNum, dayNum, hourZhi;

    if (method === '时家' || method === '日家') {
        yearZhi  = lunar.getYearZhi() || lunar.getYearInGanZhi().slice(-1);
        monthNum = lunar.getMonth();                    // ← 關鍵修正：直接取農曆月份數字（二月=2）
        dayNum   = lunar.getDay();
        hourZhi  = lunar.getTimeZhi() || lunar.getTimeInGanZhi().slice(-1);
    } else if (method === '月家') {
        yearZhi  = lunar.getYearZhi() || lunar.getYearInGanZhi().slice(-1);
        ...
        yearZhi  = lunar.getYearZhi() || lunar.getYearInGanZhi().slice(-1);
        monthNum = 2;   // 年家以立春為寅月，預設2月
        dayNum   = 1;
        hourZhi  = '子';
    }

    console.log('   取到 → 年支:', yearZhi, ' 月數:', monthNum, ' 日:', dayNum, ' 時支:', hourZhi);
    ...
    const y = diZhiToNum[yearZhi];
    const m = monthNum;        // ← 這裡直接用農曆月份數字
    const h = diZhiToNum[hourZhi];
    const d = dayNum;

    console.log('   轉換成數字 → y:', y, ' m:', m, ' d:', d, ' h:', h);

    if (!y || !m || !h || !d) {
        console.error('❌ 四柱轉換失敗');
        throw new Error(`四柱轉換失敗：年=${yearZhi} 月=${m} 日=${d} 時=${hourZhi}`);
    }
    ...
    let juNum = total % 9;
    if (juNum === 0) juNum = 9;
    console.log('   最終局數 =', juNum);

    // 5. 陰陽遁
    const month = date.getMonth() + 1;
    const dayOfMonth = date.getDate();
    const isYinJu = (month > 6 || (month === 6 && dayOfMonth >= 22)) ||
                    (month < 12 || (month === 12 && dayOfMonth >= 22));

    const juType = isYinJu ? '陽' : '陰';
    console.log('   陰陽遁 =', juType, '遁');
    console.log('【局數計算結束】\n');
    ...
    return {
        ...
        juNum,
        isYinJu,
        type: isYinJu ? 'yin' : 'yang',
        number: juNum.toString(),
        fullName: `${juType}遁${juNum}局`,
        formatCode: `${isYinJu ? 'yin' : 'yang'}-${juNum}`
    };
}
...
function analyzeGong(gongNumber, jiuXing, baMen, baShen) {
    ...
    // 門迫判斷（根據 PDF 表格）
    const menPo = (men && MEN_PO_RULES[gongStr] && MEN_PO_RULES[gongStr].includes(men)) || false;
